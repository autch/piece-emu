...
### 2.1 スレッドモデル: シングルスレッド + イベント駆動スリープ

**決定**: エミュレーションコアはシングルスレッドで実装する。

**根拠**:
- 実機は単一CPUが全デバイスを駆動しており、周辺デバイス間に密な依存関係がある
- HLT/SLEEP 時のホストCPU使用率低減が重要要件であり、シングルスレッド + イベント駆動スリープで自然に実現できる
- マルチスレッドではCPUスレッドのsuspend/resume同期コストやレースコンディションが問題になる
- 原作 P/EMU もシングルスレッドだった

**HLT/SLEEP の実装方式**:
- HALT命令検出時、「次にCPUを起こすイベント」と「その時刻」を計算
- 起床イベント: タイマ割り込み（次の発火時刻はカウンタ値とプリスケーラから計算可能）、ボタン入力（外部割り込み = SDLイベント）、計時タイマ1秒割り込み
- 次のタイマ割り込みまでの残りサイクル数をホスト時間に換算し、`SDL_WaitEventTimeout` や `nanosleep` でOSにスリープを委ねる
- ボタン入力があれば SDL イベントで即座に起床
