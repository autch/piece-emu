...
エミュレータ独自の診断機構を設ける。実機では黙って見過ごされるミスを積極的に検出し、コンパイラ・アセンブラのバグを早期発見する。

振る舞いはデバッガ接続の有無で変える：

| 診断イベント | 実機での挙動 | デバッガ未接続 | デバッガ接続中 |
|-------------|-------------|-------------|-------------|
| **未定義オペコード** | 動作未定義 | ダンプ＋エラー終了 | ブレーク（調査継続可能） |
| **マニュアルで明示禁止**（`jp.d %rb`等） | 誤動作 | ダンプ＋エラー終了 | ブレーク |
| **アドレス不整例外** | トラップベクタへ転送 | トラップベクタへ転送 | トラップ前にブレーク（条件設定可能） |
| **ext + 非対応命令**（シフト等） | ext 黙って無視 | 警告ログ＋ext破棄して続行 | ブレーク条件にできる |
| **ディレイスロットに許可リスト外命令** | 動作未定義（動く場合あり） | 警告ログ＋実行して続行 | ブレーク条件にできる |
| **バス幅不一致I/O**（HW接続と異なるアクセス幅） | バス上で無視 | 警告ログ＋続行 | ブレーク条件にできる |

**デバッガ未接続時のError**: レジスタ・PC・PSR・ALR/AHR のダンプを出力してエラー終了する。CI でテスト実行中に未定義オペコードを踏んだ場合、即座にFAILとして検出できる。

**デバッガ接続中のブレーク**: エラー終了ではなく GDB RSP の停止応答（SIGTRAP 相当）を返し、デバッガに制御を渡す。開発者（または AI エージェント）がレジスタやメモリを調査してから実行を継続・中断できる。未定義オペコードでエラー終了してしまうと「何が起きたか調べたいのに調べる前に死ぬ」ことになる。

**アドレス不整例外**: 実機のトラップ機構（PSRベクタへの転送）はそのまま再現する。デバッガ接続中はトラップ転送の直前にブレークし、デバッガ側でトラップに進むか調査するか選択できる。

**Warning の抑制**: 同一PCからの警告は初回のみ表示し、以降はカウントのみ。メインループで毎フレーム同じ警告が出ることを防ぐ。

**Warning ログの資料的価値**: 既存 P/ECE ソフトを大量に動かして、Warning が出るが正常動作するパターンを集めれば、マニュアル未記載の「実は安全に動作する命令組み合わせ」を特定するドキュメントになる。
