...
### 2.6 デバッグ専用例外（実機に存在しない）

エミュレータ独自のトラップ機構を設ける。コンパイラのバグ検出に極めて有効。

| トラップ種別 | 実機での挙動 | エミュレータ | レベル |
|-------------|-------------|-------------|--------|
| 未定義オペコード | 動作未定義 | 停止 + PC/レジスタダンプ | **Error** |
| マニュアルで明示禁止（`jp.d %rb`等） | 誤動作 | 停止 | **Error** |
| ディレイスロットに許可リスト外命令 | 動作未定義（動く場合あり） | 実行して続行 + 警告ログ | **Warning** |
| ext + 非対応命令（シフト等） | ext 黙って無視 | ext 破棄して続行 + 警告 | **Warning** |

Warning は同一PCからの初回のみ表示し、以降はカウントのみ。既存ソフトの互換性テスト時にメインループで毎フレーム警告が出ることを防ぐ。

Warning ログ自体が、マニュアル未記載の「実は安全に動作する命令」を特定するための資料になる。
