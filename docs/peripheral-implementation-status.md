...
### Step 19: Flash 書き込み (SST39VF400A / SST39VF160) + PFI 書き戻し

**ファイル**:
- [src/core/flash_device.hpp](../src/core/flash_device.hpp) — `FlashDevice` 抽象基底 + `FlatFlashRom` 既定実装
- [src/board/flash_sst39vf.hpp](../src/board/flash_sst39vf.hpp),
  [src/board/flash_sst39vf.cpp](../src/board/flash_sst39vf.cpp) — SST 39VF400A / 39VF160 モデル
- [src/system/platform_file.hpp](../src/system/platform_file.hpp),
  [src/system/platform_file.cpp](../src/system/platform_file.cpp) — POSIX (`pwrite`/`fsync`) / Win32 (`WriteFile`+`OVERLAPPED.Offset`/`FlushFileBuffers`) 抽象
- [src/system/pfi_writeback.hpp](../src/system/pfi_writeback.hpp),
  [src/system/pfi_writeback.cpp](../src/system/pfi_writeback.cpp) — debounce + 4KB セクタ単位の差分書き戻し
- [src/tests/unit/test_flash_sst39vf.cpp](../src/tests/unit/test_flash_sst39vf.cpp) — 26 ケース
- [src/tests/unit/test_pfi_writeback.cpp](../src/tests/unit/test_pfi_writeback.cpp) — 5 ケース（一時 PFI で E2E）

**設計**:

`Bus` のフラッシュ領域 (0xC00000+) は `std::unique_ptr<FlashDevice>` で抽象化されている。
既定は読み出し専用の `FlatFlashRom`（ベアメタルテスト・ELF 起動向け、書き込みは無視）。
PFI ブート時は `Bus::install_flash_device()` で `Sst39vf` に差し替えられ、CFI / 書き込みコマンドが有効化される。

`Sst39vf` は SST 39VF400A (4 Mbit / 512 KB / Device ID 0x2780) と SST 39VF160 (16 Mbit / 2 MB / Device ID 0x2782) の両方を一クラスでパラメタライズしてサポート。容量は PFI ヘッダの `pffs_end - 0xC00000` から次の 2 の累乗に切り上げて自動選択する (`Sst39vf::for_min_bytes()`)。

実装するコマンドはカーネルが実際に発行する subset のみ:

| コマンド | コード | 備考 |
|---|---|---|
| Word-Program | 0xA0 | DQ7 ポーリング不要（同期完了で 1 イテレーションで抜ける）|
| Sector-Erase | 0x30 | 4 KB |
| Block-Erase | 0x50 | 64 KB（カーネルは未使用だが対応）|
| Chip-Erase | 0x10 | （カーネルは未使用だが対応）|
| Software ID Entry | 0x90 | Manufacturer 0xBF, Device ID は容量から導出 |
| CFI Query Entry | 0x98 | Tables 5/6/7 を完備 (`device_size` は `log2(mem_size)`) |
| Short Exit | XX/F0 | カーネルが program 前後に防御的に発行 |
| Long Exit | 5555/2AAA/5555/F0 | （F0 は短形式で受理）|

不正なコマンドシーケンスは `DIE()` せず Normal 状態に静かに復帰する（フラッシュをたたくのはカーネルだけという前提下でエラーチェックが見合わないため）。Word-Program は `&=` AND マスクではなく上書きで実装（piemu と挙動を揃える）。

**Bus へのファストパス組み込み**:

`fetch16()` は `flash_device_->mem_ptr()` から直接読み出して仮想呼び出しを完全に回避する（CPU は CFI モード中に命令フェッチしないため安全）。
データ read16/read32 も `in_read_mode()` が true の間は raw memory 直読み（kernel は CFI Query を一瞬しか有効化しないため、Normal 状態のホットパスのコストが優先）。

**PFI 書き戻しのライフサイクル**:

1. `Sst39vf::set_dirty_callback()` でフックされたコールバックが、4KB セクタが clean → dirty に遷移したタイミングで steady_clock タイムスタンプを atomic 更新（CPU スレッド側）。
2. メインスレッドが `PfiWriteback::poll(now_us)` を毎フレーム呼び出し、最後の dirty イベントから `--writeback-debounce-ms` (既定 2000ms) 経過していたら、`for_each_dirty_sector()` で全 dirty セクタを `pwrite` してから `fsync`。書き戻し成功で `clear_dirty()`。
3. `cpu_thread.join()` 後に `shutdown_flush()` で残り dirty を強制 flush（debounce バイパス）。
4. SIGINT / SIGTERM でも `quit_flag` を立ててメインループを抜け、同じ shutdown 経路を踏ませる。

`FlashDevice` 基底に `sector_size()` / `any_dirty()` / `for_each_dirty_sector()` / `set_dirty_callback()` / `clear_dirty()` の仮想メソッドが置かれており、`FlatFlashRom` はすべて no-op (`sector_size() == 0`)。`PfiWriteback` は `sector_size() == 0` の device に attach されると ReadOnly に静かに降格する。

**フロントエンドごとの既定**:

| フロントエンド | 既定 | フラグ |
|---|---|---|
| `piece-emu-system` (SDL3) | writeback **ON** | `--read-only` で抑止 |
| `piece-emu` (headless) | writeback **OFF** | `--enable-flash-writeback` で有効化（テスト冪等性のための opt-in）。`--read-only` が常に優先 |

両者とも `--writeback-debounce-ms N` (0–60000) で調整可能。

**ライブラリ構成への追加**:

`piece_writeback` (静的ライブラリ) を `src/system/platform_file.cpp` + `src/system/pfi_writeback.cpp` から構築し、`piece-emu` / `piece-emu-system` 両方がリンクする。`piece_core` (FlashDevice 抽象基底) のみに依存。

**ユニットテスト**: 計 31 ケース
- Sst39vf: 状態機械・CFI Query・Software ID・dirty 追跡・容量別バリアント・コールバック発火・read8 が状態を消費しないこと
- PfiWriteback: ReadOnly が host file を変更しないこと、shutdown_flush が dirty を全て push、debounce が poll を遅延、ファイル不在時の ReadOnly 降格、ROM 接続時の ReadOnly 降格
...
1. **ADC スタブ**: カーネルが `GetVbatt()` を呼ぶ場合に追加（固定値返却）
2. **IDMA**: 将来的にシステムモードブートで必要になる可能性
3. **Overlay PFI** (Flash writeback の発展): base PFI から copy-on-write、書き込みは別 PFI へ分離。「アプリを試す → overlay を捨てる」ワークフロー
