...
**最終更新**: 2026-04-15  
**対象ディレクトリ**: `src/core/`, `src/soc/`, `src/board/`, `src/host/`, `src/debug/`  
...
HSDMA Ch1 の `on_ch1_start` コールバックから PCM バッファを SDL3 Audio Device に流す。  
T16 Ch1 の CRA タイマから PWM 周波数（約 32kHz）を算出してサンプルレートを設定する。

実装ファイル（未作成）: `src/host/audio_output.{hpp,cpp}`
...
1. **P2-4 サウンド**: HSDMA Ch1 `on_ch1_start` → SDL3 Audio callback（リングバッファ方式）
2. **P2-5 Flash 書き込み**: SST39VF400A コマンドステートマシン + PFI 書き戻し
3. **ADC スタブ**: カーネルが `GetVbatt()` を呼ぶ場合に追加（固定値返却）
