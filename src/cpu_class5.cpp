...
void Cpu::h_ld_w_sd_rs(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    special_reg(cpu.state, rd(insn)) = cpu.state.r[rs(insn)];
}
void Cpu::h_ld_w_rd_ss(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    cpu.state.r[rd(insn)] = special_reg(cpu.state, rs(insn));
}
void Cpu::h_btst(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    uint8_t byte_val = cpu.bus_.read8(cpu.state.r[rb(insn)]);
    int bit = insn & 7;
    cpu.state.psr.set_z(!((byte_val >> bit) & 1));
}
void Cpu::h_bclr(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    uint32_t ea = cpu.state.r[rb(insn)];
    int bit = insn & 7;
    cpu.bus_.write8(ea, cpu.bus_.read8(ea) & ~(1u << bit));
}
...
void Cpu::h_ld_b_rd_rs2(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    cpu.state.r[rd(insn)] = static_cast<uint32_t>(static_cast<int8_t>(cpu.state.r[rs(insn)]));
}
void Cpu::h_ld_ub_rd_rs2(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    cpu.state.r[rd(insn)] = cpu.state.r[rs(insn)] & 0xFF;
}
void Cpu::h_ld_h_rd_rs2(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    cpu.state.r[rd(insn)] = static_cast<uint32_t>(static_cast<int16_t>(cpu.state.r[rs(insn)]));
}
void Cpu::h_ld_uh_rd_rs2(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    cpu.state.r[rd(insn)] = cpu.state.r[rs(insn)] & 0xFFFF;
}
...
void Cpu::h_mlt_h(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    int32_t a = static_cast<int16_t>(cpu.state.r[rd(insn)]);
    int32_t b = static_cast<int16_t>(cpu.state.r[rs(insn)]);
...
void Cpu::h_mltu_h(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    uint32_t a = cpu.state.r[rd(insn)] & 0xFFFF;
    uint32_t b = cpu.state.r[rs(insn)] & 0xFFFF;
...
void Cpu::h_mlt_w(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    int64_t a = static_cast<int32_t>(cpu.state.r[rd(insn)]);
    int64_t b = static_cast<int32_t>(cpu.state.r[rs(insn)]);
    int64_t res = a * b;
    cpu.state.alr = static_cast<uint32_t>(res);
    cpu.state.ahr = static_cast<uint32_t>(res >> 32);
}
void Cpu::h_mltu_w(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    uint64_t a = cpu.state.r[rd(insn)];
    uint64_t b = cpu.state.r[rs(insn)];
    uint64_t res = a * b;
    cpu.state.alr = static_cast<uint32_t>(res);
    cpu.state.ahr = static_cast<uint32_t>(res >> 32);
}
void Cpu::h_mac(Cpu& cpu, uint16_t insn) {
    cpu.flush_ext();
    int rs_n = rs(insn);                  // count register
